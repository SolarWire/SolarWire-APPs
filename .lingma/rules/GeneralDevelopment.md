...
**严禁出于任何理由采用临时方案（Workaround）来解决问题或实现需求。**

### 执行要求

- **必须遵循最佳实践**：所有代码实现都应基于行业公认的最佳实践和项目既定规范。
- **提供长期解决方案**：针对任何问题或需求，必须提供稳健、可维护的长期解决方案。
- **拒绝技术债务**：不得以"先上线再说"、"以后再重构"为由引入临时性代码或补丁。
- **根因导向**：遇到问题时必须追溯根本原因,从源头解决，而非通过表面修补绕过问题。

### 例外情况

仅在以下极端情况下可考虑临时方案，但必须在代码中明确标注并创建追踪任务：
- 生产环境紧急故障修复（Hotfix），且已有明确的后续重构计划
- 外部依赖存在已知 bug 且无其他替代方案时，必须有明确的移除时间表

**任何临时方案都必须在代码审查中说明理由，并在技术债务追踪系统中登记。**
