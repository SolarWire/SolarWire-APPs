...
/**
 * 纯函数：从单个 Element 计算 ElementProps。
 * 提取出来供 useElementProps 和 useMultiElementProps 共用。
 */
export function computeElementProps(element: Element): ElementProps {
...
export function useElementProps({ element }: UseElementPropsOptions): ElementProps | null {
  return useMemo(() => {
    ...
    return computeElementProps(element);
  }, [element]);
}
