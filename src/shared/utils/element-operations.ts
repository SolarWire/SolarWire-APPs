...
export function alignElements(
  ...
  const lines = content.split(/\r?\n/);
  const elementCoords: Array<{ lineNum: number; x: number; y: number }> = [];
  ...
  switch (alignType) {
    case 'left':
      ...
      break;
    case 'right':
      targetValue = Math.max(...elementCoords.map(c => c.x));
      break;
    case 'center-h':
      const minX = Math.min(...elementCoords.map(c => c.x));
      const maxX = Math.max(...elementCoords.map(c => c.x));
    ...
    case 'top':
      ...
      break;
    case 'bottom':
      targetValue = Math.max(...elementCoords.map(c => c.y));
      break;
    case 'center-v':
      const minY = Math.min(...elementCoords.map(c => c.y));
      const maxY = Math.max(...elementCoords.map(c => c.y));
