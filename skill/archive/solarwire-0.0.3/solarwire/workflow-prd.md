...
**Five Elements Principle**: A requirement elicitation method that systematically guides AI and user from abstract business goals to concrete UI details. Each layer builds on the previous one — you cannot define scope without understanding strategy, nor structure without scope.

**Forward flow** (new features): Strategy → Scope → Structure → Framework → Presentation (abstract → concrete)
**Reverse flow** (code reverse engineering): Presentation → Framework → Structure → Scope → Strategy (concrete → abstract)

Do NOT proceed to next layer until current layer is fully understood. Ask probing questions, follow first principles.
...
| User rejects visual style | Return to Step 9, adjust presentation |
| Contradictory requirements | Ask user to clarify, do not proceed until resolved |
| Too many independent modules | Return to Phase 0 Step 2, decompose |
| PRD structure violations | Fix directly (add/remove/reorder sections), retry Check 3.5 |
| Placeholder/vague text | Fix directly, fill in missing content |
| Internal inconsistency | Fix directly, page details take highest priority |
| Ambiguous requirements | Fix by choosing one interpretation, document in Appendix |
| SolarWire syntax errors | Fix using common fixes list, re-run validation |
| Validation fails >2 times | Discard wireframe, regenerate from scratch |
| User requests feature addition | Go to Step 13, regenerate affected pages only |
| User requests logic change | Go to Step 6-7, re-confirm, then Step 13 |
| User requests major redesign | Go to Step 4, re-run Five Elements |
...
// Global UI elements (navigation, sidebar, etc.)
// Derive from project frontend code or standards.md Section 11 Layout Calculation Rules
// Each interactive element must include a note describing its behavior (see note-guide.md)

"Label Text" @(label_x, label_y)
["Element Placeholder"] @(element_x, element_y) w=width h=height bg=#FFFFFF b=#E5E7EB align=l vertical-align=m note="""Element functional description
1. Behavior section
   - When [event], [behavior]
2. Validation
   - If [condition], show '[exact error message]'"""

["Button Text"] @(button_x, button_y) w=width h=height bg=#3B82F6 c=#FFFFFF align=c vertical-align=m note="""Button functional description
1. Click action
   - When clicked, [action]
2. Success handling
   - When [success condition], [behavior]"""
```

**Note**: The container `[] @(0,0)` uses `b=#FFFFFF` to prevent the default black border. Derive layout from project frontend code; use standards.md Section 11 for coordinate calculation rules when no code exists.
...
- Refer to `standards.md` Section 10 for full incremental wireframe rules.

### Incremental Example

```solarwire
!title="[Page Name] - Changes"
!c=#111827
!size=13
!bg=#F9FAFB
...
// -- Global UI elements (UNCHANGED) --
// Derive from project frontend code or existing PRD wireframes
[] @(0,0) w=[container_width] h=[nav_height] bg=#FFFFFF b=#E5E7EB
["Element"] @(x, y) w=width h=height align=c vertical-align=m
// -- End Global UI elements --

// Page Content
"Page Title" @(title_x, title_y) size=24 bold

// Changed Elements
["Element Name"] @(x, y) w=width h=height attr=val note="""[NEW] Element functional description
1. Behavior section
   - When [event], [behavior]"""

["Element Name"] @(x, y) w=width h=height attr=val note="""[MODIFIED] Element functional description
1. NEW: [new behavior]
   - While [condition], [behavior]
2. Existing behavior unchanged
   - When [event], [behavior]"""

// Unchanged elements kept in position but without detailed notes
"Label" @(label_x, label_y)
["Element"] @(element_x, element_y) w=width h=height attr=val
```
...
5. **PRD Includes Changelog** - All PRDs must have version tracking via Change Log table
6. **Document Language** - Write documents (including titles) in the user's communication language. If unsure, ask the user.
...
9. **Multiple Approaches Comparison** - When project has multiple viable design approaches or user is uncertain, provide 2-3 approaches with trade-off analysis and recommend one (see Step 3).
