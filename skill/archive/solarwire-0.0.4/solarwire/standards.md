...
// Derive dimensions from Section 11 Page Structure table
```
...
**Priority: Derive colors from project frontend code first.** Use the Tailwind defaults below only when no project code exists.

### Default Tailwind Palette (Fallback)

| Purpose | Tailwind Shade | Hex |
|---------|---------------|-----|
...
| Alternating row | gray-50 | `#F9FAFB` |
| Borders/Lines | gray-200 | `#E5E7EB` |
...
## 9. Wireframe Coverage Rules

**Any frontend presentation that requires coding MUST have a wireframe. If it is visually a separate panel from the main page, draw it as a separate wireframe.**

### What Requires a Separate Wireframe

Any visually independent UI that requires coding: modals, drawers, popovers, dropdown menus, slide panels, action sheets, confirmation dialogs, form dialogs, detail views, etc.

### What Goes in Notes Instead

Simple transient messages that have no interactive UI: tooltips, toasts, success/error messages, loading spinners. Describe these in the triggering element's note.
...
- Apply Layout Calculation Rules (Section 11) for coordinate calculations
