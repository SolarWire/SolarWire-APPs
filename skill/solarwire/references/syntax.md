...
| `align=l|c|r|left|center|right` | enum| `l` | Rect, Circle | `align=c` |
| `v-align=t|m|b|top|middle|bottom` | enum| `t` | Rect, Circle | `v-align=m` |
...
- A SolarWire code block MAY start with `!title="页面名称"`, declaring the page name. This is a block-level directive, not an element attribute. It applies to the entire code block. If omitted, no error is raised; the page is simply rendered without a title.
...
- **Text**: Use `"text" @(x,y)` for display-only labels, static text, tags, and any non-interactive text content. Text supports ONLY `c`, `size`, `text-size`, `bold`, `opacity`, and `note`. Text does NOT support `bg`, `b`, `s`, `r`, `align`, `v-align`, `w`, `h`, or `padding-*`. Never attach layout attributes to Text elements.
...
- Text with `align`, `bg`, `b`, `s`, `v-align`, `w`, or `h` -> Remove these attributes. Text only supports `c`, `size`, `text-size`, `bold`, `opacity`, `note`.
- `w=auto` or `h=auto` -> Must use numeric values only.
- `note="content"` or `note='content'` -> Must use triple quotes `note="""content"""`. Single/double quotes for `note` are a parse error.